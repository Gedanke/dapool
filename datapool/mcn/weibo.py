...
class Weibo(object):
    ...
    def get_json(self, params):
        ...
        r = requests.get(
            url, params=params, headers=self.headers, verify=False
        )
        return r.json()
    ...
    def get_user_info(self):
        ...
        if js['ok']:
            ...
            params = {
                'containerid': '230283' + str(self.user_id) + '_-_INFO'
            }
            zh_list = [
    ...
    def get_weibo_user_info(self, user_id):
        ...
        self.get_user_info()
        res_pandas = pandas.DataFrame(self.user.items())
        res_pandas = res_pandas.set_index(0)
        return res_pandas.T
    ...
    def get_weibo(self, user_id):
        ...
        self.get_user_info()
        self.get_pages()
        df = pandas.DataFrame(self.weibo)
        df.rename(
            columns={
                "user_id": "用户ID", "screen_name": "微博名", "text": "正文", "article_url": "头条文章url", "pics": "原始图片url",
                "video_url": "视频url", "location": "位置", "created_at": "发布日期", "source": "来源",
                "attitudes_count": "点赞数", "comments_count": "评论数", "reposts_count": "转发数", "topics": "话题",
                "at_users": "@用户"
            }, inplace=True
        )
        return df
